...
scores = pvalues_idx[(pvalues_idx.shape[0]-10*N_GENES):]
...
X_data_copy = X_data
X_valid_data_copy = X_valid_data
X_test_data_copy = X_test_data

results = {'idx': [], 'train': [], 'valid': [], 'test': []}

for o in range(50):
    X_data = X_data_copy[:, :o+1]
    X_valid_data = X_valid_data_copy[:, :o+1]
    X_test_data = X_test_data_copy[:, :o+1]
    results['idx'].append(o+1)
    valid_preds = []
    test_preds = []
    ...
    results['train'].append(60 * 12 * cyclical_loss(Y_data.astype('float64'), all_preds.astype('float64')) / (Y_data.shape[0] * np.pi))
    ...
    results['valid'].append(60 * 12 * cyclical_loss(Y_valid_data.astype('float64'), valid_preds.astype('float64')) / (Y_valid_data.shape[0] * np.pi))
    ...
    results['test'].append(60 * 12 * cyclical_loss(Y_test_ang.astype('float64'), test_preds.astype('float64')) / (Y_test_ang.shape[0] * np.pi))
...
import pickle
with open('NONSDSResults3.p', 'wb') as handle:
