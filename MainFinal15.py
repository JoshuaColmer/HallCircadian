...
idx = pd.read_csv('FinalGenes.csv')
J = df.iloc[0, :].isin(idx['transcript'])
df = df.ix[:, J]
J = df_valid.iloc[0, :].isin(idx['transcript'])
df_valid = df_valid.ix[:, J]
J = df_test.iloc[0, :].isin(idx['transcript'])
df_test = df_test.ix[:, J]
X_data = df.iloc[1:, :]
X_valid_data = df_valid.iloc[1:, :]
...
# X_data = pd.concat((X_data, X_data.iloc[:, 7]), axis=1)
# X_data = pd.concat((X_data, X_data.iloc[:, 14]), axis=1)
# X_valid_data = pd.concat((X_valid_data, X_valid_data.iloc[:, 7]), axis=1)
# X_valid_data = pd.concat((X_valid_data, X_valid_data.iloc[:, 14]), axis=1)
# X_test_data = pd.concat((X_test_data, X_test_data.iloc[:, 7]), axis=1)
# X_test_data = pd.concat((X_test_data, X_test_data.iloc[:, 14]), axis=1)
...
for n_fold, (train_idx, valid_idx) in enumerate(folds.split(X_data, Y_data)):
    ...
    model.fit(X_train.astype('float64'), Y_train.astype('float64'), validation_data=(X_valid.astype('float64'), Y_valid.astype('float64')),
              batch_size=1, epochs=100, callbacks=[early_stop])  # Fit the model on the training data
