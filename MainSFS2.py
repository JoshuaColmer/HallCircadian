...
early_stop = EarlyStopping(patience=25, restore_best_weights=True, monitor='val_loss', mode='min')
...
while n_genes < N_GENES:
    ...
    # for j in tqdm.tqdm(range(2)):
        ...
        if idx1 in idx_perm:
            ...
            continue
        if counter == 1:
            idx1 = np.concatenate((np.array([idx1]), np.array(idx_perm)))
        result_iter['idx'].append(idx1)
        ...
        X_train, Y_train = X_d, Y_data

        if n_genes == 1:
            X_train = X_train.reshape(X_d.shape[0], 1)
        ...
        model = larger_model()
        model.fit(X_train.astype('float64'), Y_train.astype('float64'), validation_data=(X_v.astype('float64'), Y_valid_data.astype('float64')),
                  batch_size=3, epochs=100, callbacks=[early_stop], verbose=0)  # Fit the model on the training data
        # model.fit(train_x, train_y)
        preds = normalize(model.predict(X_d))  # Predict on the validation data
        all_preds = normalize(model.predict(X_d))
        all_valid_preds += (normalize(model.predict(X_v)) / 1)
        ...
        error += cyclical_loss(Y_data.astype('float64'), preds.astype('float64'))  # Evaluate the predictions
...
# import pickle
# with open('SFSResults2.p', 'wb') as handle:
